...
$(function() {
	...
	// Задаем ширину
		var setTimerWidth = function() {
			$('.hud-timer__inner').width($('.hud__deadline-date').width());
		};
		setTimerWidth();
		$(window).on('resize', setTimerWidth);
	// Input focus
		$(function() {
...
$(function() {
	if (!isMob()) {
			var charPool = "!<>-_\\/ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	...
	controls = new function () {
		...
		this.start = function () {
			...
			if (!isMob()) {
				...
				$('.menu-list-item a, .section-header--up, .main__year span, .main__header, .section-header').css('opacity', 1);
				$('.menu-list-item a, .section-header--up, .main__year span, .main__header, .section-header, .cover__header').addClass('js-splitme');
				...
				// Glitch
				var mainHeaderGlitch = function() {
					// $('.main__header .letter').css('opacity', 0);
				};
			 	var mainYearGlitch = function() {
					// $('.main__year .letter').css('opacity', 0);
				};
				...
				new TimelineMax()
					.to(counter, 3, {i: 35,
							onUpdate: function () {
								controls.tube = counter.i;
								controls.redraw();
							},
						ease: Power4.easeOut
					 })
					.staggerTo('.main__header .letter', 1, {onStart: mainHeaderGlitch, x: '0%', opacity: 1}, 0.05, '-=2.4')
					.staggerTo('.main__year .letter', 1, {onStart: mainYearGlitch, x: '0%', opacity: 1}, 0.05, '-=2')
					...
					.fromTo('.section-main .hud-timer', 0.6, {y: '200%', opacity: 0}, {y: '100%', opacity: 1}, '-=0.8')
			}
		...
		(function() {
			...
			// Sections
				$('.section').each(function(i, section) {
					...
					new ScrollMagic.Scene({
						offset: $section.offset().top - $('.section-padding').height() - 100,
						triggerHook: 'onEnter',
					...
					.on('enter', function() {
						...
						switch (i) {
							...
							case 2:
								TweenMax
									.to(controls, myTime, {radius: step * i,
										onUpdate: function () {
											controls.redraw();
										},
									ease: easing
			...
			// Section headers animation
				...
				$(headers).each(function(i, header) {
					...
					.on('enter', function() {
						new TimelineMax()
							.staggerTo(parentSection.find('.section-header .letter'), 0.5, {x: '0%', opacity: 1}, 0.025)
							.staggerTo(parentSection.find('.section-header--up .letter'), 0.8, {x: '0%', opacity: 1}, 0.05, '-=0.8')
							.to(parentSection.find('.section-description'), 0.9, {ease: Power1.easeInOut, y: '0%', opacity: 1}, '-=1')
					})
					.addTo(controller)
				});
					...
					$('.cover__header').each(function(i, header) {
						...
						.on('enter', function() {
							new TimelineMax()
								...
								// .staggerFromTo($(header).parent().find('.list__item'), 0.5, {ease: Power1.easeInOut, y: '60%', opacity: 0}, {ease: Power1.easeInOut, y: '0%', opacity: 1}, 0.1, '-=0.5')
						})
						.addTo(controller)
...
$(function() {
	...
	new NoiseButton({
		element: document.querySelector('.chat__btn'),
		polygon: '20, 0, 20, 0',
		wavesPos: {
			y: 0,
			x: 0
...
$(function() {
	...
	Nodes.prototype.drawImageToBackground = function() {
	  ...
	  this.bgContext.drawImage( this.bgImage, (this.canvas.width - newWidth) / 4, (this.canvas.height - newHeight) / 2, newWidth, newHeight);
...
// 		Playground.prototype._setSize = function() {
// 			this.canvas.width  = window.innerWidth;
// 			this.canvas.height = window.innerHeight;
// 		}
// 		Playground.prototype.getMousePos = function(e) {
// 			const rect = this.canvas.getBoundingClientRect();
			
// 			this.mouse.x = e.clientX - rect.left;
// 			this.mouse.y = e.clientY - rect.top;
// 		}
// 		Playground.prototype.loop = function() {
// 			window.requestAnimationFrame(this.loop.bind(this));

// 			this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
			
// 			this.customCursor.x += (this.mouse.x - this.customCursor.x) * settings.ease;
// 			this.customCursor.y += (this.mouse.y - this.customCursor.y) * settings.ease;
			
// 			this.ctx.fillStyle = '#b636c4';
// 			this.ctx.beginPath();
// 			this.ctx.arc(this.customCursor.x, this.customCursor.y, 4, 0, Math.PI*2, false);
// 			this.ctx.closePath();
// 			this.ctx.fill();

// 			if (settings.debug) {
// 			  this.ctx.strokeStyle = 'rgba(182, 54, 196, .1)';
// 			  this.ctx.beginPath();
// 			  this.ctx.moveTo(this.customCursor.x, this.customCursor.y);
...
// 			  this.ctx.strokeStyle = 'rgba(182, 54, 196, 1)';
// 			  this.ctx.beginPath();
// 			  this.ctx.arc(this.mouse.x, this.mouse.y, 12 + distance(this.customCursor.x, this.customCursor.y, this.mouse.x, this.mouse.y), 0, Math.PI*2, false);
// 			  this.ctx.closePath();
// 			  this.ctx.stroke();
// 			}
...
// 		  canvas: document.querySelector('.custom-cursor'),
// 		})
...
$(function() {
	...
	$('.request__form-member').on('click', function() {
		...
		$('.request__form-member').before('<div class="request__member"><div class="request__group request__group--num request__group--mb"><span class="member-num">' + index + '.</span><button class="member-delete" type="button"><img src="img/minus.svg" alt=""></button><div class="request__group-input"><span>Имя Фамилия</span><input class="request__input" type="text" name="member-name" required></div></div><div class="request__group"><div class="request__group-input"><span>Роль</span><input class="request__input" type="text" name="member-role" required></div></div></div>');
