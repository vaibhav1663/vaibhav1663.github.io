...
$(function() {
	...
	// Smooth scroll
		$("a[href]").on('click', function(event) {
			...
			if (hasElement) {
				...
				event.preventDefault();
				$('.login-form-wrap').addClass('leave');
				setTimeout(function() {
	...
	// Custom CSS
		var customCSS = function() {
			...
			// Timer width
			$('.hud-timer__inner').width($('.login-form-wrap .hud__deadline-date').width());
		};
		customCSS();
		$(window).on('resize', customCSS);
	// Password recovery
	$('.login-form__remember').on('click', function(e) {
		e.preventDefault();
		$('.login-form__auth, .login-form__recovery').toggleClass('is-hidden');
	})
});
...
// Input focus
$(function() {
	$('.login-form__input').on('focus', function() {
		$('.login-form__input').each(function(i, input) {
...
$(function() {
	...
	new NoiseButton({
		element: document.querySelector('.login-form__auth .login-form__btn'),
		polygon: '23, 0, 23, 0',
		wavesPos: {
			y: 0,
			x: 0
	...
	new NoiseButton({
		element: document.querySelector('.login-form__recovery .login-form__btn'),
		polygon: '23, 0, 23, 0',
		wavesPos: {
			y: 0,
			x: 0
