...
$(function() {
	...
	$('.request-page').addClass('anim');

	document.body.classList.add('page-request')
	...
	// Input enabling
		$('.project__input-edit').on('click', function() {
			// $(this).prev().prop('readonly', false);
			$(this).prev().focus();
		});
		$('.project__input[type="text"], .project__name').on('focusout', function() {
			if ($(this).attr('name') === 'video') {
				var val = $(this).val();

				if (val.indexOf('youtu') === -1 && val.indexOf('vimeo.com') === -1) {
					alert('Пожалуйста, вставьте ссылку на видео с YouTube или Vimeo.')
				}
			}
...
// Textarea autosize
$(function() {
	$('.project__name-wrap').on('change keyup keydown paste cut', 'textarea', function() {
...
$(function() {
	...
	new NoiseButton({
		element: document.querySelector('.project__preview-btn'),
		polygon: '23, 0, 23, 0',
		wavesPos: {
			y: 0,
			x: 0
	...
	new NoiseButton({
		element: document.querySelector('.project__publish-btn'),
		polygon: '23, 0, 23, 0',
		wavesPos: {
			y: 0,
			x: 0
...
$(function() {
	...
	var inputFile = document.querySelector('.project__input-file'),
...
// Add member
$(function() {
	var lastIndex = $('.project__add-member').prev().find('#member-template').prev().find('.member-num').text();
	if (lastIndex[0] === '0') {
		lastIndex = lastIndex.split('0')[1];
	}
	var index = +lastIndex + 1;


	var focusHandler = function() {
		$('.project__input, .project__name').each(function(i, input) {
	...
	$('.project__add-member').on('click', function() {
		...
		$('.project__add-member').before(memberTemplate.outerHTML);

		// При фокусе
		$('.project__add-member').prev().find('input[name="member-name"], input[name="member-role"]').on('focus', focusHandler);

		// Блочим при потери фокуса
		// $('.project__add-member').prev().find('input').on('focusout', function() {
		// 	$(this).prop('readonly', true);
		// });

		// Анблочим при клике на карандаш
		$('.project__add-member').prev().find('.project__input-edit').on('click', clickHandler);

		// Удаление инпутов
		$('.project__add-member').prev().find('.member-delete').on('click', function() {
