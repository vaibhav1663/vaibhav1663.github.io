...
$(function() {
	...
	// Input focus
		$(function() {
			$('.form__input').on('focus', function() {
				$('.form__input').each(function(i, input) {
	...
	// Reply click
		$('.comment__reply').on('click', function() {
			...
			$('.form__input[name="comment-name"]').focus();
		});
	// Comment template
		(function() {
			if ($('#comment-template').length) {
				var commentTemplate = document.querySelector('#comment-template').content.querySelector('.comment');
				commentTemplate.querySelector('.comment__author').textContent = 'Пантелеймон Христорождественский';
				$('.comments__left').append(commentTemplate);
			}
...
$(function() {
	...
	if ($('.preview__unvote-btn').length) {
		new NoiseButton({
			element: document.querySelector('.preview__unvote-btn'),
			polygon: '23, 0, 23, 0',
			wavesPos: {
				y: 0,
				x: 0
	...
	if ($('.form__send').length) {
		new NoiseButton({
			element: document.querySelector('.form__send'),
			polygon: '23, 0, 23, 0',
			wavesPos: {
				y: 0,
				x: 0
